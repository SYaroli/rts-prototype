...
## Build 04 — infantry return fire

Idle friendly infantry that take enemy damage may move up to 2 map units from their first response position to engage their attacker. Weapon ranges are unchanged. Infantry stop advancing when a shot becomes available; repeated hits cannot move the response boundary. Terrain, collision and sight rules still apply. A dead or unseen attacker, or five seconds without another hit, cancels the response. Player move/attack/stop commands clear the response; explicit movement and targeting are not interrupted. H toggles Hold Position for selected infantry, retaining normal firing without automatic advances. Reset preserves the launch-time hold stance.
